...
;; form
;;........................................................................
(defn form-heading [s][:h4.uk-heading-line.uk-text-center [:span s]])
...
(defn form-text-input [{label :label value :value :as data} {width :width} ]
  [:div {:class (u/width-trans width)}
   ...
   [:div.uk-form-controls
    [:input.uk-input
     (merge {:id "form-stacked-text" :value value} (u/data-prefix data))]]])

(defn form-select [{label :label value :value opts :options :as data} {width :width} ]
  [:div {:class (u/width-trans width)}
   [:label.uk-form-label {:for "form-stacked-text"} label]
   [:div.uk-form-controls
    (into [:select.uk-select (merge {:id "form-horizontal-select" :selected value} (u/data-prefix data))]
           (mapv (fn [s] [:option s]) opts) )]])
